      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          <img
            ...
            alt="Workflow"
          />
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">Вхід в аккаунт</h2>
        </div>
            ...
            <button 
              onClick={(e) => {
                ...
                axios_api.post("/login", {
                  ...
                  if (user.data.error) {
                    if (user.data.error instanceof Array) {
                      setState({ ...state, error: user.data.error})
                    }
                    else {
                      setState({ ...state, error: [user.data.error] });
                    }
